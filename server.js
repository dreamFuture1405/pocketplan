...
function mapGeminiError(err) {
  ...
  if (code === ERROR_CODES.UPSTREAM_401) {
    return makeErrorResponse(401, 'upstream_auth_failed');
  }
  if (code === ERROR_CODES.UPSTREAM_403) {
    return makeErrorResponse(403, 'upstream_forbidden');
  }
  if (code === ERROR_CODES.UPSTREAM_429) {
  ...
  if (code === ERROR_CODES.UPSTREAM_5XX) {
    return makeErrorResponse(502, 'upstream_error');
  }
  if (code === ERROR_CODES.MISSING_API_KEY) {
    return makeErrorResponse(500, 'server_misconfigured');
  }
  if (code === ERROR_CODES.TIMEOUT) {
    return makeErrorResponse(504, 'upstream_timeout');
  }
...
function createApp(deps) {
  ...
  const injected = deps || {};
  const validate = injected.validateInput || validateInput;
  const invokeGemini = injected.callGemini || callGemini;
  const normalizePlan = injected.normalize || normalize;
  const fallbackBuilder = injected.buildFallback || buildFallback;
